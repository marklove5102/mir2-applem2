...
//与商店NPC交易相关
  ...
  CM_COMPOUNDITEM = 138;
  CM_CUSTOM_ICON = 139;// 点击自定义图标

  RG_MINMSGINDEX = 1;
  RG_MAXMSGINDEX = 150;

  RM_TURN = 1 + RG_MAXMSGINDEX;// 转身
  RM_WALK = 2 + RG_MAXMSGINDEX;// 走
  RM_RUN = 3 + RG_MAXMSGINDEX;// 跑
  RM_HIT = 4 + RG_MAXMSGINDEX;// 击中
  RM_SPELL = 5 + RG_MAXMSGINDEX;//
  RM_PUSH = 6 + RG_MAXMSGINDEX;// 推
  RM_RUSH = 7 + RG_MAXMSGINDEX;
  RM_STRUCK = 8 + RG_MAXMSGINDEX;// 打击
  RM_DEATH = 9 + RG_MAXMSGINDEX;// 死亡
  RM_DISAPPEAR = 10 + RG_MAXMSGINDEX;// 消失
  ...
  RM_HEAR = 16 + RG_MAXMSGINDEX;// 听
  RM_DELAYPUSHED = 17 + RG_MAXMSGINDEX;// 延迟 推
  RM_CRY = 18 + RG_MAXMSGINDEX;// 哭
  RM_POISON = 19 + RG_MAXMSGINDEX;// 中毒
  RM_TRANSPARENT = 20 + RG_MAXMSGINDEX;// 透明（隐身？）
  RM_USERNAME = 21 + RG_MAXMSGINDEX;
  RM_LEVELUP = 22 + RG_MAXMSGINDEX;// 升级
  RM_CHANGENAMECOLOR = 23 + RG_MAXMSGINDEX;// 名称变色
  RM_LOGON = 24 + RG_MAXMSGINDEX;// 登录
